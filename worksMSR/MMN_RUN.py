...
from MMN_init import (  SUB, SUB_DIR, STIM_DIR,
                        # triggers
                        TRIG_STD, TRIG_DDEV, 
...
stim = preload_stim(win, RUN, STIM_DIR, SUB_DIR, device, dB_SL=60)
# Extract audio register
audio_reg = stim["audio"]  # or stim["Audio"] depending on your function

#std_sound = stim["audio"]["STD"]
#dev_sound = stim["audio"]["DEV"]
movie = stim["movie"]
...
while trial_idx < len(trials):
    ...
    if current_time >= next_sound_time:
        ...
        # --- 2. TRIGGER + SOUND PRESENTATION
        # The movie is drawn automatically via setAutoDraw(True)
        draw_pixel(win, trigger_to_RGB(current_trig)) # Draw trigger pixel on top
        # win.callOnFlip(sound_to_play.play) # sound if on laptop
        
        # Prepare audio
        infoaud_fb = audio_reg[sound_key]
        device.audio.stopSchedule()
        device.audio.setAudioSchedule(0.0, infoaud_fb['fs'], infoaud_fb['n'], 'mono')
        device.audio.setVolume(infoaud_fb['gain'])
        device.audio.setReadAddress(infoaud_fb['addr'])

        # Schedule audio to start exactly on the flip
        win.callOnFlip(device.audio.startSchedule)
        
        # Capture timestamps exactly on flip
        ...
        # Flip everything to screen (visual + trigger + audio starts now)
        win.flip()
